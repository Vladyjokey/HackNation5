...
class BaseQCAgent(ABC):
    @abstractmethod
    def extract_entity(self, hypothesis: str) -> dict: pass
    
    @abstractmethod
    def build_search_queries(self, hypothesis: str) -> List[str]: pass
    
    @abstractmethod
    def select_relevant_papers(self, hypothesis: str, papers: List[LiteratureReference]) -> List[LiteratureReference]: pass
    
    @abstractmethod
    def analyze_hypothesis(self, hypothesis: str, papers_text: str) -> dict: pass
...
class OpenAIQCAgent(BaseQCAgent):
    ...
    def extract_entity(self, hypothesis: str) -> dict:
        # Notice we use .parse() and text_format instead of .create()
...
{hypothesis}"""
        ...
        # output_parsed returns the verified Pydantic object
        # We call .model_dump() to turn it back into a dict for your pipeline.py
...
{hypothesis}"""
    ...
    def select_relevant_papers(self, hypothesis: str, papers: List[LiteratureReference]) -> List[LiteratureReference]:
        papers_text = "\n\n".join(
